...
#include "fast_resize.h"

void resize_bilinear_c1(const unsigned char *src, int srcw, int srch, int srcstep, unsigned char *dst, int w, int h, int numThreads)
{
    const int stepch = srcstep;
...
#undef SATURATE_CAST_SHORT
    ...
    int rowbufStride = FastResizeRoundUp<int>((w >> 1) + 1, 64 / sizeof(int));
    int *rowsbuf0 = new int[rowbufStride * numThreads * 2];
    int *rowsbuf1 = rowsbuf0 + rowbufStride * numThreads;

    int const dyb = (h + numThreads - 1) / numThreads;
    for (int tid = 0; tid < numThreads; tid++)
    {
        int dy0 = tid * dyb;
        int dy1 = std::min(dy0 + dyb, h);

        short *rows0 = (short *)(rowsbuf0 + rowbufStride * tid);
        short *rows1 = (short *)(rowsbuf1 + rowbufStride * tid);
        int prev_sy1 = -1;
        for (int dy = dy0; dy < dy1; dy++)
        {
            ...
            if (sy == prev_sy1)
            {
                ...
                const unsigned char *S1 = src + stepch * (sy + 1);
            ...
            else
            {
                ...
                //const unsigned char *S1 = src + srcw * (sy+1);
                const unsigned char *S0 = src + stepch * (sy);
                const unsigned char *S1 = src + stepch * (sy + 1);
...
void resize_bilinear_c3(const unsigned char *src, int srcw, int srch, int srcstep, unsigned char *dst, int w, int h, int numThreads)
{
#if __ARM_NEON
    const unsigned char *const SRC_MAX_ADD = src + srcw * srch * 3;
#endif

    assert(srcstep % 3 == 0);
    const int stepch = srcstep / 3; // channels == 3
    const int INTER_RESIZE_COEF_BITS = 11;
    const int INTER_RESIZE_COEF_SCALE = 1 << INTER_RESIZE_COEF_BITS;
    //     const int ONE=INTER_RESIZE_COEF_SCALE;
...
#undef SATURATE_CAST_SHORT
    ...
    int rowbufStride = FastResizeRoundUp<int>((w * 3 >> 1) + 3, 64 / sizeof(int));
    int *rowsbuf0 = new int[rowbufStride * numThreads * 2];
    int *rowsbuf1 = rowsbuf0 + rowbufStride * numThreads;
    ...
    for (int tid = 0; tid < numThreads; tid++)
    {
        ...
        for (int dy = dy0; dy < dy1; dy++)
        {
            ...
            if (sy == prev_sy1)
            {
                ...
                const unsigned char *S1 = src + stepch * (sy + 3);
...
void resize_bilinear_c4(const unsigned char *src, int srcw, int srch, int srcstep, unsigned char *dst, int w, int h, int numThreads)
{
    assert(srcstep % 4 == 0);
    const int stepch = srcstep / 4; // channles == 4
    const int INTER_RESIZE_COEF_BITS = 11;
    const int INTER_RESIZE_COEF_SCALE = 1 << INTER_RESIZE_COEF_BITS;
    //     const int ONE=INTER_RESIZE_COEF_SCALE;
...
#undef SATURATE_CAST_SHORT
    ...
    //short* rows0 = (short*)rowsbuf0.data;
    //short* rows1 = (short*)rowsbuf1.data;
    int rowbufStride = FastResizeRoundUp<int>(((w * 4 >> 1) + 4), 64 / sizeof(int));
    int *rowsbuf0 = new int[rowbufStride * numThreads * 2];
    int *rowsbuf1 = rowsbuf0 + rowbufStride * numThreads;
    ...
    for (int tid = 0; tid < numThreads; tid++)
    {
        ...
        for (int dy = dy0; dy < dy1; dy++)
        {
            ...
            if (sy == prev_sy1)
            {
                ...
                const unsigned char *S1 = src + srcw * (stepch + 4);
            ...
            else
            {
                ...
                //const unsigned char *S1 = src + srcw * (sy+4);
                const unsigned char *S0 = src + stepch * (sy);
                const unsigned char *S1 = src + stepch * (sy + 4);
