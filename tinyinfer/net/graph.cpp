...
std::shared_ptr<Graph> Graph::FromNet(const Net* net) {
    ...
    auto find_next_layers = [&](std::shared_ptr<BaseLayer> input_layer) -> std::vector<std::string> {
    ...
    auto find_pre_layers = [&](std::shared_ptr<BaseLayer> input_layer) -> std::vector<std::string> {
        std::vector<std::string> ret;
