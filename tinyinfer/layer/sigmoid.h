...
typedef struct SigmoidLayerParam : public BaseLayerParameter {
} SigmoidLayerParameter;

class Serializer;
class Deserializer;
class Sigmoid : public BaseLayer {
public:
  Sigmoid() : BaseLayer(LAYER_SIGMOID) {}
  Sigmoid(SigmoidLayerParameter &&param)
      : param_(std::move(param)), BaseLayer(LAYER_SIGMOID) {}
