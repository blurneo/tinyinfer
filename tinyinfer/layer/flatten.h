...
class Flatten : public BaseLayer {
 public:
    Flatten(FlattenLayerParameter &&param) : param_(param), BaseLayer(LAYER_FLATTEN) {}
    bool forward(const std::vector<std::shared_ptr<Tensor>> &input_tensors, std::vector<std::shared_ptr<Tensor>> output_tensors) override {
        CHECK_BOOL_RET(input_tensors.size(), 1, "Flatten input tensor number should be 1")
        std::shared_ptr<Tensor> input_tensor = input_tensors[0];
        std::shared_ptr<Tensor> output_tensor = output_tensors[0];
        int axis_idx = param_.axis < 0 ? param_.axis + input_tensor->dims() : param_.axis;
        CHECK_BOOL_RET(axis_idx < input_tensor->dims(), true, "Flatten axis idx exceed the dims");
        return kernel(input_tensor, axis_idx, output_tensor);
    }
 private:
    bool kernel(std::shared_ptr<Tensor> input_tensor, int axis_idx, std::shared_ptr<Tensor> output_tensor) {
        if (axis_idx == 0) {
