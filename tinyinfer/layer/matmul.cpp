...
namespace ti
{
                       ...
                       std::vector<std::shared_ptr<Tensor>> output_tensors)
  {
    ...
    return kernel_gemm(input_tensors[0], input_tensors[1], output_tensor);
  }
  bool Matmul::kernel_gemm(std::shared_ptr<Tensor> input_tensor1,
                           std::shared_ptr<Tensor> input_tensor2,
                           std::shared_ptr<Tensor> output_tensor)
  {

    int M = input_tensor1->get_h();
    int K = input_tensor1->get_w();
    const float *val_ptr1 = input_tensor1->get_values().data();
    // int K = input_tensor2->get_h();
    int N = input_tensor2->get_w();
    ...
    gemm_pp_block4x8_packab_unroll(M, K, N, val_ptr1, val_ptr2, out_ptr);
    return true;
  }
