...
class GlobalAveragePool : public BaseLayer {
 public:
    GlobalAveragePool(GlobalAveragePoolLayerParameter &&param) : param_(std::move(param)), BaseLayer(LAYER_GLOBAL_AVERAGE_POOL) {}
    bool forward(const std::vector<std::shared_ptr<Tensor>> &input_tensors, std::vector<std::shared_ptr<Tensor>> output_tensors) override {
        CHECK_BOOL_RET(input_tensors.size(), 1, "GlobalAveragePool input tensor number should be 1")
